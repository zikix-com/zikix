...
class XLSXWriter
{
    ...
    public static function xmlspecialchars($val)
    {
        ...
        return strtr(htmlspecialchars($val, ENT_QUOTES | ENT_XML1),
                     $badchars,
        ...
        foreach ($this->cell_styles as $i => $cell_style_string) {
            ...
            if (isset($style['border']) && is_string($style['border']))//border is a comma delimited str
            {
                ...
                if (isset($style['border-color']) && is_string($style['border-color']) && $style['border-color'][0] == '#') {
                    ...
                    $v                     =
                        strlen($v) == 3 ? $v[0] . $v[0] . $v[1] . $v[1] . $v[2] . $v[2] : $v;// expand cf0 => ccff00
                                       ...
                                       $freeze_columns = false)
    {
        ...
        $max_cell                  =
            XLSXWriter::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL);//XFE1048577
        ...
        if ($sheet->freeze_rows && $sheet->freeze_columns) {
            ...
            $sheet->file_writer->write('<selection activeCell="' . self::xlsCell($sheet->freeze_rows,
                                                                                 0) . '" activeCellId="0" pane="topRight" sqref="' . self::xlsCell($sheet->freeze_rows,
                                                                                                                                                   0) . '"/>');
    ...
    protected function writeCell(XLSXWriterBuffererWriter &$file,
                                                          $row_number,
                                                          $column_number,
                                                          $value,
                                                          $num_format_type,
    ...
    public function writeSheetRow($sheet_name, array $row, $row_options = null)
    {
        ...
        foreach ($row as $v) {
            ...
            $cell_style_idx     =
                empty($style)
                    ? $sheet->columns[$c]['default_cell_style']
                    : $this->addCellStyle($number_format,
                                          json_encode(isset($style[0]) ? $style[$c] : $style));
